    ...
    for (NodeContainer::Iterator j = endDevices.Begin(); j != endDevices.End(); ++j)
    {
        ...
        if (enableRTX)
        {
            ...
            if (rxPower > *edSensitivity)
            {
                ...
                endDeviceLoraPhy->SetSpreadingFactor((uint8_t)7);
                NS_LOG_DEBUG("rxPower: " << rxPower << " edSensitivity: " << (*edSensitivity) );
                NS_LOG_DEBUG("sf7asdasdasdasd: " << static_cast<int>(endDeviceLoraPhy->GetSpreadingFactor()));
            }
            else if (rxPower > *(edSensitivity + 1))
            {
                mac->SetDataRate(4);
                sfQuantity[1] = sfQuantity[1] + 1;
                endDeviceLoraPhy->SetSpreadingFactor((uint8_t)8);
                NS_LOG_DEBUG("sf8: " << static_cast<int>(endDeviceLoraPhy->GetSpreadingFactor()));
            }
            else if (rxPower > *(edSensitivity + 2))
            {
                mac->SetDataRate(3);
                sfQuantity[2] = sfQuantity[2] + 1;
                endDeviceLoraPhy->SetSpreadingFactor((uint8_t)9);
                NS_LOG_DEBUG("sf9: " << static_cast<int>(endDeviceLoraPhy->GetSpreadingFactor()));
            }
            else if (rxPower > *(edSensitivity + 3))
            {
                mac->SetDataRate(2);
                sfQuantity[3] = sfQuantity[3] + 1;
                endDeviceLoraPhy->SetSpreadingFactor((uint8_t)10);
                NS_LOG_DEBUG("sf10: " << static_cast<int>(endDeviceLoraPhy->GetSpreadingFactor()));
            }
            else if (rxPower > *(edSensitivity + 4))
            {
                mac->SetDataRate(1);
                sfQuantity[4] = sfQuantity[4] + 1;
                endDeviceLoraPhy->SetSpreadingFactor((uint8_t)11);
                NS_LOG_DEBUG("sf11: " << static_cast<int>(endDeviceLoraPhy->GetSpreadingFactor()));
            }
            else if (rxPower > *(edSensitivity + 5))
            {
                mac->SetDataRate(0);
                sfQuantity[5] = sfQuantity[5] + 1;
                endDeviceLoraPhy->SetSpreadingFactor((uint8_t)12);
                NS_LOG_DEBUG("sf12: " << static_cast<int>(endDeviceLoraPhy->GetSpreadingFactor()));
            }
            else
            { // Device is out of range. Assign SF12.
                ...
                endDeviceLoraPhy->SetSpreadingFactor((uint8_t)12);
                NS_LOG_DEBUG("sf13: " << static_cast<int>(endDeviceLoraPhy->GetSpreadingFactor()));
            }
        }
        else
        {
            ...
            Ptr<GatewayLoraPhy> gatewayPhy =
                gatewayLoraNetDevice->GetPhy()->GetObject<GatewayLoraPhy>();
            ...
            if (rxPower > *gwSensitivity)
            {
                ...
                sfQuantity[0] = sfQuantity[0] + 1;
                endDeviceLoraPhy->SetSpreadingFactor((uint8_t)7);
                NS_LOG_DEBUG("sf7: " << static_cast<int>(endDeviceLoraPhy->GetSpreadingFactor()));
            }
...
std::vector<uint16_t>
LorawanMacHelper::SetSpreadingFactorsStrategies(NodeContainer endDevices,
                                                std::vector<uint16_t> sfQuantity,
                                                uint32_t edge,
                                                uint32_t edge2,
                                                uint32_t nDev,
                                                uint8_t mode)
...
std::vector<uint16_t>
LorawanMacHelper::SetSpreadingFactorsProp(NodeContainer endDevices,
                                          double prop1,
                                          double prop2,
                                          double rad)
{
